...
import { removeMovie } from '../../store/watchListSlice';
  ...
  return (
    <div>
        <div className='flex justify-center gap-6 flex-wrap'>
          {
            genere.map(genre =>{
              return <button key={genre} onClick={chooseGenre} className='transition-all hover:scale-110  duration-300 hover:shadow-[0_0_25px_#facc15] hover:border-yellow-400 cursor-pointer rounded-4xl' >{genreConfiguration[genre]}</button>
            })
          }
        </div>
              ...
              <tbody>
                {
                  moviesInDisplay.map((movie)=>{
                    return <tr key={movie.id} className='border  font-bold'>
                      ...
                      <td  className='items-center pr-3 cursor-pointer'>
                        <FontAwesomeIcon onClick={()=>{dispatch(removeMovie(movie.id))}} size='lg' icon={faTrash} style={{ color: '#fa0000' }} /> 
                      </td>
                    </tr>
                  })
                }
